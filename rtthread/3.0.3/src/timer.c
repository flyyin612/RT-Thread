...
/**
	*该函数将停止一个定时器
	*
	*@param timer将要被停止的定时器
	*	
	*@retval 操作状态,RT_EOK on OK,-RT_ERROR on error
	*/
rt_err_t rt_timer_stop(rt_timer_t timer)
{
	register rt_base_t level;
	
	/*只有active的定时器才能被停止，否则退出返回错误*/
...
rt_err_t rt_timer_start(rt_timer_t timer)
{
	...
	/*获取timeout tick,最大的timeout tick不能大于RT_TICK_MAX/2*/
	timer->timeout_tick = rt_tick_get() + timer->init_tick;
	
	/*关中断*/
	level = rt_hw_interrupt_disable();
	
	/*将定时器插入到定时器列表，获取系统定时器列表根节点地址，rt_timer_list
	是一个全局变量*/
	...
	{
		/*列表不为空，当没有定时器被插入到系统定时器列表时，该村换不执行*/
...
/*
*@该函数用于扫描系统定时器列表，当有超时事件发生时就调用对应的超时函数
*
